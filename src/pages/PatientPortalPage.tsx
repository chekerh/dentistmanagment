      ...
      <header className="sticky top-0 z-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 md:px-10 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-slate-900 dark:text-white">DentalCare Patient Portal</h1>
          </div>
          <div className="flex items-center gap-3">
            <button
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-10 py-8 flex flex-col gap-8">
        ...
        <div className="flex items-center gap-5 bg-gradient-to-r from-primary to-blue-500 rounded-2xl p-6 text-white shadow-lg">
          ...
          <div>
            ...
            <p className="text-white/70 text-sm mt-1">Patient ID: {patient.id}</p>
          </div>
          <div className="ml-auto hidden sm:flex flex-col items-end gap-1">
            <p className="text-white/80 text-xs">Next Appointment</p>
            <p className="text-white font-semibold">{patient.nextAppointment ?? 'None scheduled'}</p>
          </div>
        </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: 'Total Visits', value: appts.filter(a => a.status === 'completed').length, icon: 'check_circle', color: 'text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400' },
                ...
                { label: 'Balance Due', value: `$${patient.balance.toFixed(0)}`, icon: 'payments', color: patient.balance > 0 ? 'text-red-600 bg-red-50 dark:bg-red-900/20 dark:text-red-400' : 'text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400' },
              ].map((s) => (
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800">
                ...
                <button onClick={() => setActiveTab('appointments')} className="text-sm text-primary hover:text-primary-hover">View all</button>
              </div>
              {upcomingAppts.length === 0 ? (
                ...
                <div key={a.id} className="flex items-center gap-4 px-5 py-4 border-b border-slate-100 dark:border-slate-800 last:border-0">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-slate-900 dark:text-white">{getTreatmentLabel(a.treatmentType)}</p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
              <h3 className="font-bold text-slate-900 dark:text-white mb-4">My Information</h3>
              ...
              {patient.allergies.length > 0 && (
                <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Known Allergies</p>
        ...
        {activeTab === 'appointments' && (
          <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-slate-900 dark:text-white">My Appointments</h3>
              <button
                onClick={() => setShowBookModal(true)}
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              ...
              {treatments.length === 0 ? (
                ...
                <div key={t.id} className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 last:border-0">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-semibold text-slate-900 dark:text-white">{getTreatmentLabel(t.type)}</p>
                      <p className="text-xs text-slate-500 mt-0.5">{t.date} • {t.dentist}</p>
            ...
            {payments.map((pay) => (
              <div key={pay.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm flex items-center justify-between gap-4">
                <div>
                  ...
                  {pay.insuranceClaim && <p className="text-xs text-blue-500 mt-0.5">Claim: {pay.insuranceClaim}</p>}
                </div>
                <div className="flex flex-col items-end gap-1">
      ...
      {showBookModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-6 w-full max-w-md">
            ...
            <form onSubmit={(e) => { e.preventDefault(); alert('Appointment request sent! The clinic will confirm shortly. (Demo)'); setShowBookModal(false); }} className="flex flex-col gap-4">
              ...
              <label className="flex flex-col gap-1.5">
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Preferred Time</span>
              ...
              <label className="flex flex-col gap-1.5">
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Reason for Visit</span>
                <select className="h-10 border border-slate-200 dark:border-slate-700 rounded-lg px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 bg-white dark:bg-slate-800 text-slate-900 dark:text-white">
                  {['Routine Checkup', 'Cleaning', 'Tooth Pain', 'Follow-up', 'Other'].map(r => <option key={r}>{r}</option>)}
                </select>
              </label>
              <label className="flex flex-col gap-1.5">
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Additional Notes</span>
