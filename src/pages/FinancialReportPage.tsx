...
const REVENUE_CATEGORIES = [
  { label: 'Orthodontics', value: 15400, pct: 75, color: 'bg-primary' },
  { label: 'Implants', value: 18200, pct: 90, color: 'bg-purple-500' },
  { label: 'General', value: 8500, pct: 45, color: 'bg-indigo-500' },
  { label: 'Cosmetic', value: 5200, pct: 30, color: 'bg-pink-500' },
  { label: 'Hygiene', value: 2100, pct: 15, color: 'bg-teal-500' },
];
    ...
    <Layout>
      ...
      <div className="p-6 max-w-[1200px] mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 pb-4 border-b border-slate-200 dark:border-border-dark">
          <div>
            <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Financial Report</h1>
            <p className="text-slate-500 dark:text-text-secondary mt-1">March 2026 • Detailed revenue breakdown</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            {
              label: 'Total Revenue', value: `$${totalRevenue.toLocaleString()}`, trend: '+12%', trendUp: true,
              sub: 'vs last month', icon: 'payments', iconColor: 'text-primary',
            },
            {
              label: 'Pending Payments', value: `$${totalPending.toLocaleString()}`, trend: '-5%', trendUp: false,
              sub: `${mockPayments.filter(p => p.status === 'pending').length} invoices pending`, icon: 'pending_actions', iconColor: 'text-orange-500',
            },
            {
              label: 'Overdue', value: `$${totalOverdue.toLocaleString()}`, trend: '+2%', trendUp: true,
              sub: 'Requires immediate attention', icon: 'warning', iconColor: 'text-red-500',
            },
          ].map(card => (
            ...
            <div className="flex-1 flex flex-col justify-center gap-5">
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-700 mt-2">
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Most pending payments are from insurance claims processing.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                      ...
                      <td className="px-6 py-4 text-right">
                        {pay.status === 'paid' ? (
                          ...
                          <button className="text-xs font-semibold h-8 px-3 rounded bg-primary text-white hover:bg-primary-hover transition-colors shadow-sm shadow-primary/20">
                            {pay.status === 'overdue' ? 'Resend Invoice' : 'Issue Invoice'}
                          </button>
                        )}
                      </td>
                    </tr>
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
            <p className="text-xs text-slate-400">{filteredPayments.length} transaction(s)</p>
            <Link to="/billing" className="text-sm text-primary hover:text-primary-hover font-medium flex items-center gap-1">
              View All Billing <span className="material-symbols-outlined text-lg">arrow_forward</span>
            </Link>
          </div>
        </div>
      </div>
